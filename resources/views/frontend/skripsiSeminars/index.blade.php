        ...
        <div class="col-md-12">
            @can('skripsi_seminar_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.skripsi-seminars.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $skripsiSeminar->id }}">
                                        ...
                                        <td>
                                            @can('skripsi_seminar_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.skripsi-seminars.show', $skripsiSeminar->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('skripsi_seminar_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.skripsi-seminars.edit', $skripsiSeminar->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('skripsi_seminar_delete')
                                                <form action="{{ route('frontend.skripsi-seminars.destroy', $skripsiSeminar->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
