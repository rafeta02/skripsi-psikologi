    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.applications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('stage') ? 'is-invalid' : '' }}" name="stage" id="stage">
                    ...
                    @foreach(App\Models\Application::STAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('stage', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('stage'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('submitted_at') ? 'is-invalid' : '' }}" type="text" name="submitted_at" id="submitted_at" value="{{ old('submitted_at') }}">
