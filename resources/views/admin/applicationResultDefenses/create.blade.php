...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.applicationResultDefense.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.application-result-defenses.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('final_title_en') ? 'is-invalid' : '' }}" type="text" name="final_title_en" id="final_title_en" value="{{ old('final_title_en', '') }}" maxlength="500">
                @if($errors->has('final_title_en'))
                    <span class="text-danger">{{ $errors->first('final_title_en') }}</span>
                @endif
                <span class="help-block">Judul akhir dalam Bahasa Inggris (opsional)</span>
            </div>
            <div class="form-group">
