...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.applicationAssignment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.application-assignments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lecturer') ? 'is-invalid' : '' }}" name="lecturer_id" id="lecturer_id">
                    @foreach($lecturers as $id => $entry)
                        <option value="{{ $id }}" {{ old('lecturer_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('assigned_at') ? 'is-invalid' : '' }}" type="text" name="assigned_at" id="assigned_at" value="{{ old('assigned_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('responded_at') ? 'is-invalid' : '' }}" type="text" name="responded_at" id="responded_at" value="{{ old('responded_at') }}">
