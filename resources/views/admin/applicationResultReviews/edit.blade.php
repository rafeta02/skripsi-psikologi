...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.applicationResultReview.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.application-result-reviews.update", [$applicationResultReview->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="application_id">{{ trans('cruds.applicationResultReview.fields.application') }}</label>
                <select class="form-control select2 {{ $errors->has('application') ? 'is-invalid' : '' }}" name="application_id" id="application_id">
                    @foreach($applications as $id => $entry)
                        <option value="{{ $id }}" {{ (old('application_id') ? old('application_id') : $applicationResultReview->application->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('application'))
                    <span class="text-danger">{{ $errors->first('application') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.applicationResultReview.fields.application_helper') }}</span>
            </div>
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.applicationResultReview.fields.result_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="revision_deadline">{{ trans('cruds.applicationResultReview.fields.revision_deadline') }}</label>
                <input class="form-control date {{ $errors->has('revision_deadline') ? 'is-invalid' : '' }}" type="text" name="revision_deadline" id="revision_deadline" value="{{ old('revision_deadline', $applicationResultReview->revision_deadline) }}">
                @if($errors->has('revision_deadline'))
                    <span class="text-danger">{{ $errors->first('revision_deadline') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.applicationResultReview.fields.revision_deadline_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="form_document">{{ trans('cruds.applicationResultReview.fields.form_document') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.applicationResultReview.fields.form_document_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="latest_script">{{ trans('cruds.applicationResultReview.fields.latest_script') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.applicationResultReview.fields.latest_script_helper') }}</span>
            </div>
            <div class="form-group">
...
Dropzone.options.formDocumentDropzone = {
    url: '{{ route('admin.application-result-reviews.storeMedia') }}',
    maxFilesize: 10, // MB
    addRemoveLinks: true,
    headers: {
    ...
    init: function () {
@if(isset($applicationResultReview) && $applicationResultReview->form_document)
          var files =
            {!! json_encode($applicationResultReview->form_document) !!}
              for (var i in files) {
              var file = files[i]
...
<script>
    ...
    init: function () {
@if(isset($applicationResultReview) && $applicationResultReview->latest_script)
      var file = {!! json_encode($applicationResultReview->latest_script) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
