...
@if($applicationId)
    <div class="alert alert-success {{ $class ?? 'mb-0' }}">
        <h6 class="font-weight-bold mb-2">
            <i class="fas fa-certificate mr-1"></i> Dokumen Kelulusan
        </h6>
        <p class="mb-2 small">
            Proses skripsi sudah <strong>selesai (finalisasi admin)</strong>. Unduh dokumen berikut:
        </p>
        <div class="d-flex flex-wrap gap-2">
            <a class="btn btn-sm btn-success" href="{{ route('pdf.surat-keterangan-lulus', $applicationId) }}" target="_blank">
                <i class="fas fa-file-pdf mr-1"></i> Surat Keterangan Lulus
            </a>
            <a class="btn btn-sm btn-outline-success" href="{{ route('pdf.transkrip-nilai', $applicationId) }}" target="_blank">
